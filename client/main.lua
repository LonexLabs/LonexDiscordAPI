...
-- ============================================================================
-- ACTIVITY SYSTEM CLIENT (Blips, Weapon Clearing)
-- ============================================================================

local ActivityBlips = {}  -- [source] = blipHandle
local FlashingBlips = {}  -- [source] = true (for siren flashing)

---Remove all activity blips
...
local function UpdateActivityBlips(blipData)
    ...
    for _, data in ipairs(blipData) do
        ...
        if data.source ~= mySource then
            ...
            -- Show heading indicator (direction cone)
            if showHeading then
                ShowHeadingIndicatorOnBlip(blip, true)
                -- Set initial rotation if we have heading data
                if data.heading then
                    SetBlipRotation(blip, math.floor(data.heading))
                end
            end
            
            -- Track if this blip should flash (siren active)
...
-- Give loadout when going on duty
    ...
    if loadout.Weapons then
        for _, weaponData in ipairs(loadout.Weapons) do
            if weaponData.weapon then
                ...
                -- Apply attachments if specified
                ...
                -- Apply tint if specified (0-7 for most weapons)
...
-- Clear weapons when going off duty
...
-- Clear armor when going off duty
...
-- Track and send vehicle/siren status to server
    ...
    while true do
        ...
        if Config.ActivitySystem and Config.ActivitySystem.Enabled then
            ...
            if inVehicle then
                local vehicle = GetVehiclePedIsIn(ped, false)
                if vehicle and vehicle ~= 0 then
                    -- Check if vehicle has sirens and they're on
...
-- Flashing blips for units with sirens active
CreateThread(function()
    local flashState = false
    
    while true do
        Wait(300)  -- Flash interval
        ...
        if Config.ActivitySystem and Config.ActivitySystem.Enabled then
            ...
            for src, blip in pairs(ActivityBlips) do
                if DoesBlipExist(blip) and FlashingBlips[src] then
                    -- Alternate between red (1) and blue (3)
                    if flashState then
...
-- Update blip positions periodically (client-side interpolation between server syncs)
CreateThread(function()
    while true do
        Wait(500)
        ...
        if Config.ActivitySystem and Config.ActivitySystem.Enabled and Config.ActivitySystem.Blips and Config.ActivitySystem.Blips.Enabled then
            ...
            -- Update blip positions and headings for smoother movement
            for src, blip in pairs(ActivityBlips) do
                if DoesBlipExist(blip) then
                    ...
                    if targetPlayer ~= -1 then
                        ...
                        if DoesEntityExist(targetPed) then
                            ...
                            -- Update heading/rotation if enabled
                            if showHeading then
